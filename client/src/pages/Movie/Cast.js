...
const Cast = ({ cast }) => {
    ...
    return (
        <Container className={classes.container}>
            {cast.slice(0,8).map((elem,k) => (
                <Paper key={k} className={classes.actor}>
                    <img src={`https://image.tmdb.org/t/p/w185${elem.profile_path}`} alt={elem.name} className={classes.actorImage} />
                    <Typography style={{fontWeight:'900', fontSize:'14px', textAlign:'center'}}>{elem.original_name} </Typography>
